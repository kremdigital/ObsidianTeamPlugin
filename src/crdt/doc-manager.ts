...
export const REMOTE_ORIGIN = Symbol('obsidian-sync-remote');
export const DISK_ORIGIN = Symbol('obsidian-sync-disk');
export const EDITOR_ORIGIN = Symbol('obsidian-sync-editor');
...
export interface DocManagerOptions {
  ...
  /**
   * Database-name builder. Defaults to `obsidian-sync-{bindingId}-{slug}`,
...
function defaultDbName(bindingId: string, filePath: string): string {
  ...
  return `obsidian-sync-${bindingId}-${slug}`;
}
